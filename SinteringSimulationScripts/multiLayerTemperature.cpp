...
void MPIprint1D (double *rhoP, string namestart, string nameend, double eval_time, struct variables dim, int zpos, int ypos, int zheight, int ysize)
{
ostringstream oss;
string var;
char *filename;
oss<<namestart<<eval_time<<nameend;
...
double RaND, etaSqSum, etaCubeSum, rhoSq, rhoQuad, A2, B12, therm;
...
int procSize;
int zheight, area,ysize,zpos,ypos, areay, fullarea;

//defining temperature variables
double *TEMP, *templeft, *tempright, *tempup, *tempdown;

//Modification eta_product instead of etasum
double etaij = 0.0;

//ffile 0-no input from file 1-input from file (continuing along so full eta file)
//keep as 1 for multiple layers
ffile = 1;

//variable to control if to read in from temperature file
int thermalGradient = 1; 
...
//storing temperature information
map<double, double*> tempMap;
double T0[5], T450[5], T500[5], T550[5];

//[0] -> con.A     [1] -> con.B    [2] -> con.gbenergy   [3] -> con.surfenergy  [4] -> con.surfdif  con.gbdif = con.surfdif/10  con.voldif = con.surfdif/1000
T0[0] = 0.0;        T0[1] = 0.0;        T0[2] = 0.0;        T0[3] = 0.0;        T0[4] = 0.0;
T450[0] = 6.25;     T450[1] = 2.6317;   T450[2] = 3.5498;   T450[3] = 15.081;   T450[4] = 43.814;
T500[0] = 10.15;    T500[1] = 2.5;      T500[2] = 4.4;      T500[3] = 16.8;     T500[4] = 66.5;
T550[0] = 8.03;     T550[1] = 4.1;      T550[2] = 6;        T550[3] = 23.12;    T550[4] = 71.3;
...
psizey = 26;//8 //set size/number of processors per z slice
psizez = 26;//8 //set number of z slices
...
fullarea = dim.xlength*dim.ylength;
...
MPI_File_open(MPI_COMM_WORLD,"newLayer.dat",amode,MPI_INFO_NULL,&fh);

for (int z = 0; z<zheight;z++)
{
ti = area*z*(dim.noOfparticles+1);
disp = (z*fullarea + ypos*area + zpos*dim.ylength*areay)*sizeof(size_dob)*(dim.noOfparticles+1);
...
//Get temperature profile and store in arrays
if(thermalGradient)
{
    //allocating temperature arrays
    TEMP = store1D(procSize);
    templeft = store1D(areay);
    tempright = store1D(areay);
    tempup= store1D(area);
    tempdown= store1D(area);
    ...
    MPI_File_open(MPI_COMM_WORLD,"tempProfile.dat",amode,MPI_INFO_NULL,&fh);

    //reading in bulk of processor's temperature
    for (int z = 0; z<zheight;z++)
    {
    ti = dim.xlength*ysize*z;
    disp = (z*fullarea + ypos*area + zpos*dim.ylength*areay)*sizeof(size_dob);
    MPI_File_read_at(fh,disp,&TEMP[ti],area,MPI_DOUBLE,&status);
    }

    //reading in z edges
    if (zpos != 0)
    {
        disp = (ypos*area + (zpos*zheight-1)*fullarea)*sizeof(size_dob);
        MPI_File_read_at(fh,disp,&tempdown[0],area,MPI_DOUBLE,&status);
    }
    else
    {
       for (int ct = 0; ct<areay; ct++)
       {
            tempdown[ct] = 0;
       } 
    }
    ...
    if (zpos != (psizez-1))
    {
        disp = (ypos*area + (zpos+1)*zheight*fullarea)*sizeof(size_dob);
        MPI_File_read_at(fh,disp,&tempup[0],area,MPI_DOUBLE,&status);   
    ...
    if (ypos != 0)
    {
        ...
        {
            ti = dim.xlength*z;
            disp = (z*fullarea + (ypos*ysize-1)*dim.xlength + zpos*fullarea*zheight)*sizeof(size_dob);
            MPI_File_read_at(fh,disp,&templeft[ti],dim.xlength,MPI_DOUBLE,&status);
        }
    }
    else
    {
    ...
    if (ypos != (psizey-1))
    {
        ...
        {
            ti = dim.xlength*z;
            disp = (z*fullarea + (ypos+1)*area + zpos*fullarea*zheight)*sizeof(size_dob);
            MPI_File_read_at(fh,disp,&tempright[ti],dim.xlength,MPI_DOUBLE,&status);
        }   
    }
    else
    {
    ...
    //check initital temperature
    /*
    MPIprint1D(TEMP,"tempAll",".dat",t,dim,zpos,ypos,zheight,ysize);
    print1D(templeft,"templeft",".txt",areay,prank);
    print1D(tempright,"tempright",".txt",areay,prank);
    print1D(tempup,"tempup",".txt",area,prank);
    print1D(tempdown,"tempdown",".txt",area,prank);
    */
}
...
therm = tempup[a];
con.A = tempMap[therm][0];
con.B = tempMap[therm][1];
con.gbenergy = tempMap[therm][2];
con.surfenergy = tempMap[therm][3];
con.surfdif = tempMap[therm][4];
con.gbdif = con.surfdif/10;
con.voldif = con.surfdif/1000;
