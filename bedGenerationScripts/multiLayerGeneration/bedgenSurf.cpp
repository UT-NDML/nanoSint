...
int overlappingSurface(double *Xx, double R, double K, double **SurfInfo, int nosurfpts, int *surfPts)
{
    double tmpmag, tmppart[3];
    int sct = 0;

    for (int ii = 0; ii < nosurfpts; ii++)
    {
        tmppart[0] = Xx[0]-SurfInfo[ii][0];
        tmppart[1] = Xx[1]-SurfInfo[ii][1];
        tmppart[2] = Xx[2]-SurfInfo[ii][2];
...
//cout<<fixed;
    ...
    //for reading surface info
    double **SurfInfo, conv, tash;
    int *surfPts, nosurfpts, ptsSearch;
    ...
    res = 0.99; //restitution coefficients have to be between 0 and 1, not including 0
    resw = 0.99;
    ...
    ifstream partFileS ("ASurfacePoints10.txt"); //read in from surface file

    for (int i = 1; i <= nosurfpts; i++)
    {
        int j = i-1;
        for (int k = 1; k<=3; k++)
        {
    ...
    while (t<endTime)
    {
        ...
        surfPts = store1Dint(nosurfpts);
        Initint1D(surfPts,nosurfpts);
        ...
        for (int a = 0; a < noParticles; a++)
        {
            ...
            //Surface points Calculation
            Initint1D(surfPts,nosurfpts);

            //cout<<"Particle =>"<<a<<"\n";
            ptsSearch = overlappingSurface(Xi, ri, K, SurfInfo, nosurfpts, surfPts);

            for (int sp = 0; sp < ptsSearch; sp++)
            {
                int spT = surfPts[sp];

                transfr = SurfInfo[spT][0];
                Xj[0] = transfr;
                transfr = SurfInfo[spT][1];
                Xj[1] = transfr; 
                transfr = SurfInfo[spT][2];
                Xj[2] = transfr;
